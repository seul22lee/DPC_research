...
def sigmoid(x,u_ub, u_lb):
    sig = Sigmoid()
    return sig(x*1e-3)*(u_ub-u_lb) + u_lb
...
def RMPC_obj_wo_constraint(u_hat0:torch.tensor, # future u values within the control horizon, length = M; this should be the warm start version
                ...
                x_past:torch.tensor, 
                x_past_depth:torch.tensor,# past state that changes
    ...
    # u_hat_sig = sigmoid(u_hat0, torch.ones_like(u_hat0), -torch.ones_like(u_hat0))
    # u_hat = u_hat_sig.reshape(-1,1)
    # u_hat_in = u_hat_sig.unsqueeze(0)
    
    x_hat = NN_Nominal.forward(u_hat, u_future_fix, u_past, x_past_fix, x_past.transpose(1,0))[:,0]
    ...
    Obj = 1 * torch.sum((x_hat-torch.tensor(SP_hat,dtype=torch.float32))**2) + 5*torch.sum((u_hat1[:-1]-u_hat_temp)**2)
    relu = ReLU()
