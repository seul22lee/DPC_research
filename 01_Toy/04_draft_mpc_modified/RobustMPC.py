...
class RMPC_iteration:
    ...
    def run_one_step(self,
        ...
        if solution.success == False:
            ...
            print(f"[Step {self.count}] warm start 실패 → zeros 재시도: {solution.message}")
            if solution.success == False:
    ...
    def run_one_step_w_penalty(self,
        ...
        _t0 = time.time()
        solution = minimize(self.optim_obj, self.u_hat, method = "SLSQP", args=(u_past_s, x_past_s, SP_hat_s, self.P, self.NN_model, self.tube_model, self.error_past, self.x_max, self.x_min, self.u_max, self.u_min),jac=True, bounds=bounds, constraints = constraint, options={'maxiter':200,'gtol':1e-3})
        
        if solution.success == False:
            solution = minimize(self.optim_obj, np.zeros((self.P)), method = "SLSQP", args=(u_past_s, x_past_s, SP_hat_s, self.P, self.NN_model, self.tube_model, self.error_past, self.x_max, self.x_min, self.u_max, self.u_min),jac=True, bounds=bounds, constraints = constraint, options={'maxiter':200,'gtol':1e-3})
            print(f"[Step {self.count}] warm start 실패 → zeros 재시도: {solution.message}")
            if solution.success == False:
                # generate sobol sampling from -1 to 1
                n_sobol = 20
        ...
        self.optim_time_log.append(time.time() - _t0)
        u_opt_original_scale = (solution.x + 1).reshape(-1,1)*0.5*(self.u_max-self.u_min).reshape(-1,1) + self.u_min.reshape(-1,1)
        
        
        
        # Nominal Model (forward pass 1회만)
        TiDE_output_for_save = self.NN_forward(solution.x,u_past_s,x_past_s,SP_hat_s,self.P,self.NN_model)[0].detach().numpy()
        TiDE_output = TiDE_output_for_save[0,:].reshape(-1,1)  # only the first element
        x_hat_next = (TiDE_output + 1).reshape(-1,1) * 0.5 * ((self.x_max-self.x_min).reshape(-1,1)) + self.x_min.reshape(-1,1)
        self.nominal_output = np.concatenate((self.nominal_output,x_hat_next),axis=1)
        self.nominal_u = np.concatenate((self.nominal_u,u_opt_original_scale[0].reshape(-1,1)))
        x_hat_horizon = ((TiDE_output_for_save + 1).reshape(-1,2) * 0.5 * ((self.x_max-self.x_min).reshape(-1,2)) + self.x_min.reshape(-1,2))[:,0].reshape(1,-1)
        self.x_hat_horizon_save = np.concatenate((self.x_hat_horizon_save,x_hat_horizon),axis=0)
        ...
        # System Model (nominal u 직접 적용)
        e = self.x_sys_current - self.x_hat_current
        self.e_save = np.concatenate((self.e_save,e),axis=1)
        u_applied = u_opt_original_scale[0].reshape(-1,1)
        # Saturation
        if u_applied >= self.u_max:
            u_applied = np.ones_like(u_applied) * self.u_max
        if u_applied <= self.u_min:
            u_applied = np.ones_like(u_applied) * self.u_min 
    ...
    def u_tightening(self, lb, ub):
        ...
        past_series = torch.concat((error_past, u_past), dim=2)  # (1, window, 3)
        past_series_tube = past_series[:, -1:, :]  # tube model은 window=1 → 마지막 1스텝만

        with torch.no_grad():
            error_hat = self.tube_model([past_series_tube, u_hat, None])[0,:,:,:] # scaled pred median of x1 and x2, shape = [10,2,3]
        ...
        # lb > ub 방지: tightening이 너무 심하면 원래 bounds로 클리핑
        u_lower = np.maximum(u_lower, lb)
        u_upper = np.minimum(u_upper, ub)
        # 그래도 역전되면 원래 bound 사용
        invalid = u_lower >= u_upper
        u_lower[invalid] = lb[invalid]
        u_upper[invalid] = ub[invalid]
