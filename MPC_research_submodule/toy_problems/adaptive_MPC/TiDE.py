...
def apply_lora_to_model(model, rank=4, alpha=1.0, opt = 'all_linear_layers'):
    if opt == 'all_linear_layers':
        for name, module in model.named_children():
            if isinstance(module, nn.Linear):
                setattr(model, name, LoRALinear(module, rank=rank, alpha=alpha))
            else:
                apply_lora_to_model(module, rank=rank, alpha=alpha)
    else: # lora for the last layer (self.lookback_skip)
        raise ValueError('option not identified')
    
def fine_tune_last_layer(model):
    for name, module in model.named_children():
        if "lookback_skip" in name:
...
def add_last_layer(model):
    class AugmentedModel(nn.Module):
        def __init__(self, model):
            ...
            # Retrieve output dimensions from the base model's attributes
            self.T = self.base.output_chunk_length
            self.D = self.base.output_dim
            self.N = self.base.nr_params

            # Flattened size for final linear layer
            flattened_dim = self.T * self.D * self.N
            self.linear_head = nn.Linear(flattened_dim, flattened_dim)
            
            for param in self.linear_head.parameters():
                param.requires_grad = True

            # Initialize linear layer as identity
        ...
        def forward(self, x):
            ...
            # Apply identity-initialized linear layer
            x_out = self.linear_head(x_flat)

            # Reshape back to (B, T, D, N)
            return x_out.view(B, self.T, self.D, self.N)

    return AugmentedModel(model)

def fine_tune_add_LP(model):
    ...
    return FineTunedModel(model)


def fine_tune_hybrid(model):
    class FineTunedHybridModel(nn.Module):
        def __init__(self, model):
            super().__init__()
            self.base = model
    ...
    return FineTunedHybridModel(model)
