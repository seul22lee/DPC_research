...
class GAMMA_MPC():
    ...
    def MPC_run_one_step_pytorch(self):
        ...
        #     mp_temp_ref_part_s[cut_off_indicator:] = mp_temp_ref_part_s[cut_off_indicator-1]
                # Policy prediction
        with torch.no_grad():
            mp_temp_past_t = self.x_past.T.unsqueeze(0)
            laser_past_t = self.u_past.view(1, -1, 1)
            fix_cov_past_t = fix_cov_past.unsqueeze(0)

            fix_cov_past_s = self.TiDE.scaler_x(fix_cov_past_t, dim_id=[0, 1, 2])
            laser_past_s = self.TiDE.scaler_x(laser_past_t, dim_id=[3])
            mp_temp_past_s = self.TiDE.scaler_y(mp_temp_past_t, dim_id=[0, 1])

            mp_temp_past_s = mp_temp_past_s.permute(1,0,2)
            #print(fix_cov_past_s.shape, laser_past_s.shape, mp_temp_past_s.shape)
            policy_in_past = torch.cat((fix_cov_past_s, laser_past_s, mp_temp_past_s), dim=2)

            fix_cov_future_t = fix_cov_future.unsqueeze(0)
            fix_cov_future_s = self.TiDE.scaler_x(fix_cov_future_t, dim_id=[0, 1, 2])
            mp_temp_ref_t = torch.tensor(mp_temp_ref, dtype=torch.float32, device=self.device).view(1, self.P, 1)
            mp_temp_ref_s = self.TiDE.scaler_y(mp_temp_ref_t, dim_id=[0])
            depth_bounds = torch.tensor([[0.1423, 0.4126]] * self.P, dtype=torch.float32, device=self.device).reshape(1, self.P, 2)
            
            mp_temp_ref_s = mp_temp_ref_s.permute(1,0,2)
            #print(fix_cov_future_s.shape, mp_temp_ref_s.shape, depth_bounds.shape)
            policy_in_future = torch.cat((fix_cov_future_s, mp_temp_ref_s, depth_bounds), dim=2)

            u_seq_pred = self.policy_model((policy_in_past, policy_in_future))
            self.warm_start = u_seq_pred[0].unsqueeze(0)
            print("warm_start : " , self.warm_start.flatten())


        # optimization
        time1 = time.time()
        solution_s = pytorch_minimize(lambda u:self.obj(u,fix_cov_future_s,past_laser_power_s,fix_cov_past_s, mp_temp_past_part_s,mp_temp_ref_part_s,self.P,self.TiDE),self.warm_start,method="l-bfgs",return_all=True)                           
        ...
        #print("solution_s : ", solution_s.x[0][0].shape, solution_s.x[0][0])
        #print("self.warm_start[0] : ", self.warm_start[0][0].shape, self.warm_start[0][0])

        if solution_s.success != True or solution_s.x[0][0]==self.warm_start[0][0]:
            solution_s = pytorch_minimize(lambda u:self.obj(u,fix_cov_future_s,past_laser_power_s,fix_cov_past_s, mp_temp_past_part_s,mp_temp_ref_part_s,self.P,self.TiDE),torch.ones((self.P,1))*-0.8,method="l-bfgs",return_all=True)                           
            time2 = time.time()
        ...
        if rmpc:
            ...
            #print("solution[0] shape : ", solution[0].shape)
        ...
        #self.x_past_save_actual = torch.concatenate((self.x_past_save_actual,torch.tensor([x_current,depth_current]).reshape(1,-1)))
