...
class GAMMA_MPC():
    def __init__(self,
        ...
        self.begin_dx = 0
        self.begin_dy = 0
        self.end_layer_counter = 0
        self.start_layer_counter = 0
        self.open_loop_control_sequence = 0
    ...
    def MPC_run_one_step_pytorch(self):
        ...
        self.current_z = fix_cov_future_s[0,0]
        future_z_in_horizon = fix_cov_future_s[0:self.P,0]
        
        
        # ========= correct past covariates at the beginning of each layer ======
        if torch.any(fix_cov_past_s[:,0] != fix_cov_past_s[0,0]) and torch.all(self.current_z >= fix_cov_past_s[:,0]) and torch.all(self.current_z == future_z_in_horizon):
            print(self.current_z)
            print(fix_cov_past_s[:,0])
            print(fix_cov_past_s[:,1])
            print(fix_cov_past_s[:,2])
            print("this is the beginning of the layer")
            if self.start_layer_counter == 0:
                self.begin_z = fix_cov_future_s[0,0]
                self.begin_dx = fix_cov_future_s[0,1]
                self.begin_dy = fix_cov_future_s[0,2]
            
            fix_cov_past_s[:self.P-self.start_layer_counter,0] = self.begin_z
            fix_cov_past_s[:self.P-self.start_layer_counter,1] = self.begin_dx
            fix_cov_past_s[:self.P-self.start_layer_counter,2] = self.begin_dy
            print(fix_cov_past_s[:,0])
            print(fix_cov_past_s[:,1])
            print(fix_cov_past_s[:,2])
            print(self.start_layer_counter)
            
            self.start_layer_counter +=1
        
        else:
            self.start_layer_counter = 0
        
        # ========= do open-loop at the end of the layer ============
        # Do open-loop control if the end of the layer is met
        if torch.any(future_z_in_horizon != self.current_z):
            if self.end_layer_counter == 0:
                #self.open_loop_control_sequence = solution
                fix_cov_future_s[:,0] = self.current_z
                print(self.current_z)
            else:
                #solution = self.open_loop_control_sequence[min(self.end_layer_counter,self.P-1)] 
                fix_cov_future_s[:,0] = self.current_z   
            self.end_layer_counter += 1
        else:
            self.end_layer_counter = 0
