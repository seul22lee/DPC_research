...
class GAMMA_MPC():
    def __init__(self,
        ...
        self.warm_start = torch.zeros((self.P,1))
        
        # save data
        self.x_past_save = x_past.transpose(1,0) # [50,2]
        self.u_past_save = u_past.reshape(-1,1)
        self.NN_pred_save = x_past.transpose(1,0) #[50,2]
        self.save_time = []
    ...
    def MPC_run_one_step_pytorch(self):
        ...
        # scale past features
        mp_temp_past_part_s = self.TiDE.scaler_y(self.x_past[0,:]) # shape = [50,2]
        mp_depth_past_part_s = self.TiDE_depth.scaler_y(self.x_past[1,:]) # shape = [50,2]
        ...
        # optimization
        time1 = time.time()
        solution_s = pytorch_minimize(lambda u:self.obj(u,fix_cov_future_s,past_laser_power_s,fix_cov_past_s, mp_temp_past_part_s, mp_depth_past_part_s ,mp_temp_ref_part_s,self.P,self.TiDE,self.TiDE_depth),self.warm_start,method="l-bfgs")                             
        time2 = time.time()

        if (self.global_state != None) and (self.LossParam != None):
            self.global_state.reset_global_state()
            self.LossParam.reset()
        
        # scale solution to original scale
        solution = self.TiDE.inv_scaler_x(solution_s.x, dim_id = [3])
        self.warm_start = solution_s.x
        
        with torch.no_grad():
            # predict MP temp
            mp_hat_opt_s = self.TiDE.forward(solution_s.x, fix_cov_future_s, past_laser_power_s, fix_cov_past_s, mp_temp_past_part_s.transpose(1,0)) 
            mp_hat_depth_opt_s = self.TiDE_depth.forward(solution_s.x, fix_cov_future_s, past_laser_power_s, fix_cov_past_s, mp_depth_past_part_s.transpose(1,0)) 
            # scale MP temp to original scale
            mp_hat_opt = self.TiDE.inv_scaler_y(mp_hat_opt_s) # [50,1]
            mp_hat_depth_opt = self.TiDE_depth.inv_scaler_y(mp_hat_depth_opt_s)
            ...
            mp_hat_save = torch.concatenate((mp_hat_opt[0],mp_hat_depth_opt[0])).reshape(-1,2)
            ...
            self.NN_pred_save = torch.concatenate((self.NN_pred_save,mp_hat_save))
